...
import shelve
import sys
#import refactored.lab_db as ldb
from  . import lab_pg as lpg
...
def _get_key_name(fullname, db):
    ...
    """
    if db.get_key(fullname):
        return fullname
    init_name = _mk_init_name(fullname)
    if db.get_key(init_name):
        return init_name
    return None
...
class DBFinder:
    ...
    def __init__(self, path_entry):
        ...
        # will be used.
        if DBFinder._maybe_recursing:
            raise ImportError
        try:
            # Test the path_entry to see if it is a valid shelf
            try:
                DBFinder._maybe_recursing = True
                #with shelve.open(path_entry, 'r'):
                #    pass
            finally:
                DBFinder._maybe_recursing = False
    ...
    def find_module(self, fullname, path=None):
        ...
        if key_name:
            ...
            return DBLoader(path)
        print('  not found : ', key_name)
        return None
...
class DBLoader:
    ...
    def get_source(self, fullname):
        ...
        try:
            ...
            key_name = _get_key_name(fullname, db)
            if key_name:
                return lpg.get_source(key_name)
            raise ImportError(
    ...
    def is_package(self, fullname):
        init_name = _mk_init_name(fullname)
        return lpg.get_key(init_name) != None
